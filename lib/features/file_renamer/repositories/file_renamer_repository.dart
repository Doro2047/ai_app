...
class FileRenamerRepository {
  ...
  Future<List<RenamePreview>> executeRename(
    ...
    final validPreviews = previews.where((p) => p.hasChange && !p.hasConflict).toList();
    ...
    for (final preview in previews) {
      ...
      if (preview.hasConflict) {
        results.add(preview.copyWith(error: '名称冲突，跳过'));
        continue;
      }
    ...
    for (final preview in previews) {
      ...
      try {
        ...
        if (await file.exists()) {
          ...
          results.add(preview.copyWith(error: '文件不存在，无法撤销'));
        }
      } catch (e) {
        results.add(preview.copyWith(error: '撤销失败: $e'));
      }
    }
