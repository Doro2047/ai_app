...
class FileScannerBloc extends FeatureBlocBase<FileScannerEvent, FileScannerState> {
  ...
  void _onScanCompleted(
    ...
    _addLog(emit, '扫描完成: 发现 ${event.results.length} 个文件, 总大小 ${stats.totalSizeFormatted}');
  ...
  Future<void> _onExportRequested(
    ...
    if (state.filteredResults.isEmpty) {
      emit(state.copyWith(error: '没有可导出的数据'));
      return;
    }

    emit(state.copyWith(isExporting: true, clearError: true));
    _addLog(emit, '正在导出扫描结果...');
