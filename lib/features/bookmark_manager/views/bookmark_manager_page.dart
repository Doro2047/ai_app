...
class _BookmarkManagerViewState extends State<_BookmarkManagerView> {
      ...
      builder: (context, state) {
        return AppScaffold(
          ...
          statusBarText: state.isLoading
              ? '正在加载书签...'
              : state.hasBookmarks
                  ? '共 ${state.totalUrls} 个链接, ${state.totalFolders} 个文件夹'
                  : '就绪',
                ...
                child: state.isLoading
                    ...
                    : !state.hasBookmarks
                        ? const EmptyState(
                            ...
                            title: '暂无书签',
                            description: '导入 Edge 浏览器书签文件以开始管理',
                          )
  ...
  Future<void> _selectFile() async {
    try {
      ...
      if (!mounted) return;
      AppToast.show(context, message: '选择文件失败: $e', type: AppToastType.error);
    }
  }

  void _loadBookmarks() {
    if (_selectedFilePath.isEmpty) {
      AppToast.show(context, message: '请先选择书签文件', type: AppToastType.warning);
      return;
    }
  ...
  Future<void> _exportBookmarks() async {
    try {
      ...
      if (savePath != null) {
        ...
        if (mounted) {
          AppToast.show(context, message: '书签已导出', type: AppToastType.success);
        }
      }
    } catch (e) {
      if (!mounted) return;
      AppToast.show(context, message: '导出失败: $e', type: AppToastType.error);
    }
  }
}
